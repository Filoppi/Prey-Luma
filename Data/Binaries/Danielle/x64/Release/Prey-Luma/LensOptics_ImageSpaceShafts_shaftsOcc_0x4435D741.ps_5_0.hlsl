  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.x = depthMap.Sample(depthMap_s, v2.xy * LumaData.RenderResolutionScale).x; // LUMA FT: fixed depth map not being scaled by rendering resolution scale ("MapViewportToRaster()")
  r0.x = -v1.z + r0.x;
  r0.y = cmp(0 < r0.x);
  r0.x = cmp(r0.x < 0);
