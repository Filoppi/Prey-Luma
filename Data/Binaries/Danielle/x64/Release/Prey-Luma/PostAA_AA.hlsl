...
Texture2D<float> PostAA_DepthTex : register(t2); // LUMA FT: unused. This is the near/far direct depth (0 being the near (or possibly, but not likely, a zero distance from the camera), and 1 being the far)
Texture2D<float2> PostAA_VelocityObjectsTex : register(t3);
Texture2D<float> PostAA_DeviceDepthTex : register(t16); // LUMA FT: This is the inverse depth (1 being the near (or possibly a zero distance from the camera), and 0 being the far). This is actually a R24G8_TYPELESS, and it's the depth used by the stencil (I think)
