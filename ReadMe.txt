...
Vcpkg versioning is very unclear to me. If you simply specify the baseline vcpkg version in the json manifest, their documentation claims that simply determines the minimum version of the libraries to use, so to avoid issues down the line, I've forced all packages to the version I tested the mod on (specifying the baseline version might have been enough?).
DKUtil also has its own vcpkg dependencies, and they don't seem to be fully version locked, so that might cause problems in the future.
